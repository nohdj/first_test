    ...
    stages {
        ...
        stage('Checkout') {
            steps {
                ...
                git branch: 'main', url: 'https://github.com/nohdj/first_test.git'
            }
        }
        ...
        stage('Deployment Image to Update') {
            steps {
                script {
                    ...
                    sh "kubectl set image deployment/group1-team1-spring-boot-jenkins group1-team1-spring-boot-jenkins=${REGISTRY}/${IMAGE_NAME}:${IMAGE_TAG} --namespace=${NAMESPACE}"
                }
            }
        }
    }
